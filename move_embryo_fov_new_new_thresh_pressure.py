...
def move_embryo_fov_new_new_thresh_pressure(fov,action,X_est,Y_est,Z_est,inj_depth,inj_speed,back_pressure_value,pressure_value,pressure_time,arduino,current_x,current_y,current_z,view_1_x,view_1_y,view_2_x,view_2_y,footage_socket_1,footage_socket_2,footage_socket_3,inj_num,y1_1_crop,y2_1_crop,x1_1_crop,x2_1_crop,y1_2_crop,y2_2_crop,x1_2_crop,x2_2_crop,im_height_1,im_width_1,im_height_2,im_width_2,graph,sess,ser,time_wait,next_z,post_z,current_z_new,move_num,pic,thresh_1,thresh_2,resize,sum_image_thresh_min,target_pixel,x_coord_emb_1,y_coord_emb_1,x_coord_emb_2,y_coord_emb_2,send_num,q,inj_pressure,pressure_value_current):
    ...
    if fov==1 and action=='centroid':
        ...
        if 1 not in list_classes:
            ...
            #prototype
            # dz=int(float(Delta_Z_FOV_1_lin(view_1_y,yc_rc[0][list_classes_index]+y1_1_crop-100)))
        ...
        if 1 not in list_classes:
            ...
            # dz=int(float(Delta_Z_FOV_2_lin(view_2_y,yc_rc[0][list_classes_index]+y1_2_crop-100)))
            current_z=current_z+dz
            print('Current Z = ',current_z)   
        ...
        next_z=current_z
            ...
            if 1 in list_classes:
                ...
                dz_1=int(float(Delta_Z_FOV_1_lin(view_1_y,yc_rc[0][list_classes_index]+y1_1_crop-130)))
                # dz_1=int(float(Delta_Z_FOV_1_lin(view_1_y,yc_rc[0][list_classes_index]+y1_1_crop-100)))
                x_coord_emb_1=xc_rc[0][list_classes_index]+x1_1_crop
                y_coord_emb_1=yc_rc[0][list_classes_index]+y1_1_crop
                x_coord_tip_1=view_1_x
                y_coord_tip_1=view_1_y
                ...
                if 1 in list_classes:
                    ...
                    dz_2=int(float(Delta_Z_FOV_2_lin(view_2_y,yc_rc[0][list_classes_index_2]+y1_2_crop-100)))
                    # dz_2=int(float(Delta_Z_FOV_2_lin(view_2_y,yc_rc[0][list_classes_index_2]+y1_2_crop-100)))
            ...
            # if 3 in list_classes:
                ...
                # if 3 in list_classes:
                    ...
                    if abs(x_coord_emb_1-x_coord_tip_1)<=14 and abs(x_coord_emb_2-x_coord_tip_2)<=14:
                        current_x=current_x
                        current_y=current_y
                    else:
                        current_x=current_x_new
        ...
        if 1 in list_classes_1 and 1 in list_classes_2:
        # if 3 in list_classes_1 and 3 in list_classes_2:
            print('Posterior detected FOV1')
            # list_classes_index_1=list_classes_1.index(3)
            list_classes_index_1=list_classes_1.index(1)
            if resize==1:
            ...
            list_classes_index_2=list_classes_2.index(1)
            if resize==2:
            ...
            time.sleep(1)
            if inj_pressure==0:
                # while pressure_value<=50 and injected==2:
                ...
                pressure_value_new=1
                while pressure_value_current!=pressure_value_new and injected==2:
                    correct=0
                    o=0
                    while correct==0 and injected==2:
                        print('Try ',o+1)
                        signal=continuous_pressure(back_pressure_value_new,pressure_value_new,'inj')
                        arduino.write(signal.encode())
                        arduino.flush()
                        q_=arduino.readline()
                        q_=q_.decode()
                        s=q_.find('Received')
                        if pressure_value_new>0:    
                        # if pressure_value_new>5:
                            img1,img2=queue_image(q)
                            sum_image_1=detect_injection(img1,int(x_post_1+x1_1_crop),int(y_post_1_new+y1_1_crop),inj_num,1)
                            sum_image_2=detect_injection(img2,int(x_post_2+x1_2_crop),int(y_post_2_new+y1_2_crop),inj_num,2)
                            sum_image=np.max([sum_image_1,sum_image_2])
                        ...
                        if pressure_value_new>5:
                            press_num=int((pressure_value_new + 43.6279)/0.9535)
                        else:
                            press_num=int((pressure_value_new + 0.35)/0.107)
                        if pressure_value_new>5:
                            if press_num>99:
                                if q_[s+9]=='P' and q_[s+10:s+13]==str(int((pressure_value_new + 43.6279)/0.9535)) and q_[s+13]=='p' and q_[s+14]=='\r':
                                    correct=1
                                else:
                                    o+=1
                            else:
                                if q_[s+9]=='P' and q_[s+10:s+12]==str(int((pressure_value_new + 43.6279)/0.9535)) and q_[s+12]=='p' and q_[s+13]=='\r':
                                    correct=1
                                else:
                                    o+=1 
                        else:
                            if press_num>9:
                                if q_[s+9]=='P' and q_[s+10:s+12]==str(int((pressure_value_new + 0.35)/0.107)) and q_[s+12]=='p' and q_[s+13]=='\r':
                                    correct=1
                                else:
                                    o+=1 
                            else:
                                if q_[s+9]=='P' and q_[s+10:s+11]==str(int((pressure_value_new + 0.35)/0.107)) and q_[s+11]=='p' and q_[s+12]=='\r':
                                    correct=1
                                else:
                                    o+=1 
                    if sum_image_thresh_min<=sum_image<=target_pixel or sum_image>=target_pixel:
                ...
                while injected==2 and timer<10:
                    ...
                    if sum_image_thresh_min<=sum_image<=target_pixel or sum_image>=target_pixel:
                        injected=1
                    end_time_injection=time.time()
                    timer+=end_time_injection-start_time_injection
                pressure_value=pressure_value_new
            
            # # Come out
            current_z=current_z-inj_depth-300
            XYZ_Location(20000,20000,100,current_x,current_y,current_z,ser)
            #new
